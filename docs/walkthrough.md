...
Input: phase 3 output, the curated drug-target map (`assets/drug_target_map.tsv`), the live DGIdb cache (`assets/dgidb_drugs.tsv`, 198 rows over 21 genes), and the live ClinicalTrials.gov cache (`assets/ctgov_rms_drugs.tsv`). Joins each annotated event to all drugs whose targets match. Each drug-event pair carries a `drug_evidence_score` that combines phase-of-development weight with pediatric-evidence weight (yes_approved > pediatric_trial > adult_only > preclinical). Output is long-format: one row per variant-drug pair, so a single hotspot mutation can fan out into many candidate drugs.
...
3. **Tier classification in phase 5** (per-row tiers shipped v0.14). Each phase-5 row now carries a `tier` column: Tier 1 (FDA-approved drug for a DRIVER variant), Tier 2 (phase 2/3 drug for a DRIVER variant), Tier 3 (phase 1 / preclinical drug for a DRIVER variant), or untiered (non-DRIVER). Per-sample reports group by tier; the cohort summary has a tier-rollup table. Cohort-prevalence gating per the v2 plan §5 (Tier 1 -> >5% of subtype, Tier 2 -> >3%) and an `STS_committee_portfolio.md` deliverable are queued for v0.15.
