...
Input: phase 2 output, the DepMap RMS summary, and the OpenPedCan expression summary. Adds a subtype-aware `dependency_score` from real DepMap 26Q1 Chronos values across 14 RMS cell lines, plus an `expression_score` that captures RMS-vs-other-pediatric-tumor specificity from real OpenPedCan v15 RNA-seq (16 RMS samples, 4108 non-RMS pediatric samples). An oncogene-addiction floor lifts known driver dependencies (FGFR4 V550L, CDK4 amp, etc.) above what the cell-line average alone would suggest. v0.17 adds a third axis on real cohorts: `bin/opentargets_client.py` queries the OpenTargets v4 GraphQL API for the disease's full associated-target table (one cached call per disease, ~3000 rows) and writes per-gene `opentargets_score` plus a `opentargets_disease` label into the phase-3 output. The score is informational in v0.17 (shown in per-sample reports and the STS portfolio, not in the confidence formula).
...
4. **Live OpenTargets disease-association** (shipped v0.17 in phase 3). Was deferred at v0.6 because per-target queries did not expose disease filters; v0.17 instead queries the disease side once per cohort run (`disease(efoId).associatedTargets`, paginated), caches the full gene-score map in `data/opentargets_cache/<efo>.json`, and exposes per-gene `opentargets_score` + `opentargets_disease` columns in phase-3 output. Informational in v0.17; v0.18 may integrate it into the confidence formula.
