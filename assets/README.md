...
| `openpedcan_expression_summary.tsv` | Per-gene log2(TPM+1) RMS-vs-other-pediatric z-score and bounded expression score | Auto-generated by `bin/fetch_openpedcan_expression.py` from OpenPedCan v15 RSEM TPM matrix (`gene-expression-rsem-tpm-collapsed.rds`) and `histologies.tsv`; 16 RMS RNA-seq samples vs 4108 other pediatric. Phase 3 reads this, Phase 5 uses the resulting `expression_score` to fill the 0.15 expression weight. | Refresh whenever OpenPedCan publishes a new release |
